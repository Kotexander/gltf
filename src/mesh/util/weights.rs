...
impl<A: Cast> Iterator for CastingIter<'_, A> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match &mut self.0 {
            ReadWeights::U8(i) => i.next().map(A::cast_u8),
            ReadWeights::U16(i) => i.next().map(A::cast_u16),
            ReadWeights::F32(i) => i.next().map(A::cast_f32),
        }
    }
    ...
    fn nth(&mut self, x: usize) -> Option<Self::Item> {
        match &mut self.0 {
            ReadWeights::U8(i) => i.nth(x).map(A::cast_u8),
            ReadWeights::U16(i) => i.nth(x).map(A::cast_u16),
            ReadWeights::F32(i) => i.nth(x).map(A::cast_f32),
        }
    }
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        match &self.0 {
            ReadWeights::U8(i) => i.size_hint(),
            ReadWeights::U16(i) => i.size_hint(),
            ReadWeights::F32(i) => i.size_hint(),
        }
    }
}
